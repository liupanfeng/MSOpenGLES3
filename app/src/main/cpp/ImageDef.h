...
public:
	static void AllocNativeImage(NativeImage *pImage)
	{
		...
		switch (pImage->format)
		{
			...
			case IMAGE_FORMAT_P010:
			{
				...
				break;
			default:
				LOGD("NativeImageUtil::AllocNativeImage do not support the format. Format = %d", pImage->format);
				break;
		}
	}
	...
	static void CopyNativeImage(NativeImage *pSrcImg, NativeImage *pDstImg)
	{
		...
		switch (pSrcImg->format)
		{
			...
			default:
			{
				LOGD("NativeImageUtil::CopyNativeImage do not support the format. Format = %d", pSrcImg->format);
			}
				break;
		}
	...
	static void DumpNativeImage(NativeImage *pSrcImg, const char *pPath, const char *pFileName)
	{
		...
		LOGD("DumpNativeImage fp=%p, file=%s", fp, imgPath);
	...
	static void LoadNativeImage(NativeImage *pSrcImg, const char *pPath)
	{
		...
		LOGD("LoadNativeImage fp=%p, file=%s", fp, pPath);
		int dataSize = 0;
		if(fp)
		{
			switch (pSrcImg->format)
			{
				...
				default:
				{
					LOGD("LoadNativeImage not support the format %d.", pSrcImg->format);
					break;
				}
			}
