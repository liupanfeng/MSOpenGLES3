...
static void* createThreadLock(void);            //创建线程锁
static int waitThreadLock(void *lock);          //等待线程锁
static void notifyThreadLock(void *lock);       //通知线程锁
static void destroyThreadLock(void *lock);      //销毁线程锁
static void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context);   //音频播放回调
static void bqRecorderCallback(SLAndroidSimpleBufferQueueItf bq, void *context);  //音频录制回调


// creates the OpenSL ES audio engine  创建 OpenSL ES 音频引擎
...
// opens the OpenSL ES device for output  打开 OpenSL ES 设备进行输出
...
// Open the OpenSL ES device for input  打开 OpenSL ES 设备进行输入
...
// close the android audio device //关闭安卓音频设备
...
// gets a buffer of size samples from the device 从设备获取大小样本的缓冲区
...
// this callback handler is called every time a buffer finishes playing 每次缓冲区播放完毕时都会调用此回调处理程序
...
// puts a buffer of size samples to the device  将大小样本的缓冲区放入设备
